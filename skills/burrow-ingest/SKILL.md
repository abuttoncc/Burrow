...
3. Read the target domain's ontology contract at `wiki/<domain>/_ontology.md`. **If its `status` is `draft` or the file is missing: refuse to compile into that domain and say why.** Fail loud.
...
Run the seven steps of the gate protocol, in order: route → register source → extract → canonical lookup → tier adjudication → stage write plan → consult `08-Ops/approval-ledger.md` and promote or queue.
...
- New pages and merges follow the format visible in `wiki/macro/indicators/Reserve Requirement Ratio.md` (frontmatter, State zipper, Facts, Relations, Changelog). Record `reviewer: gate-auto` or the queued status in the changelog.

## Queue cards

For each queued item write `08-Ops/review/<date>-<slug>.md`:
...
Record the run in `08-Ops/runs/` (one file per run; in-session compiles may append to today's record), move compiled Inbox items to `Inbox/_compiled/`, and report: applied / queued / rejected, with one line each. If the queue grew, mention it once — do not nag.
