...
<p align="center"><img src="docs/dashboard-prototype.png" alt="Burrow 控制面板：韦斯莱时钟、器物清单、飞轮轨迹、审核队列、信任影子闸" width="920"></p>
<p align="center"><sub>晨间视图——韦斯莱时钟（器物状态）、昨夜飞轮轨迹、审核队列、信任账本。<a href="docs/dashboard-prototype.html">可交互原型 →</a></sub></p>
...
```
/burrow-init        # 引导式开荒：骨架检查 → 领域契约访谈 → 立常设问题
```
...
> 完整上手指南（含 cron 权限坑、lite→引擎迁移、两周训练期预期）：[docs/getting-started.md](docs/getting-started.md)（英文）
...
| `burrow-init` | **引导开荒**：骨架检查 → 模式检测 → 领域契约访谈 → 立常设问题 |
...
顶部 hero 图就是正在迭代的晨间视图——**韦斯莱时钟**（每根指针是一个器物，扇区是状态：工作中/排队/待命/暂停/异常）+ 飞轮轨迹 + 审核队列 + 可见的信任账本。可交互 HTML 原型：[docs/dashboard-prototype.html](docs/dashboard-prototype.html)——可以实际批准/驳回卡片看账本变化、暂停器物看时钟指针摆动。
